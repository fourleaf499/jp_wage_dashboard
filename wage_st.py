...
df_jp_ind = pd.read_csv(
    "雇用_医療福祉_一人当たり賃金_全国_全産業.csv", encoding="shift_jis"
)
df_jp_category = pd.read_csv(
    "雇用_医療福祉_一人当たり賃金_全国_大分類.csv", encoding="shift_jis"
)
df_pref_ind = pd.read_csv(
    "雇用_医療福祉_一人当たり賃金_都道府県_全産業.csv", encoding="shift_jis"
)

st.header("■2019年：一人当たり平均賃料のヒートマップ")
...
df_pref_map = pd.merge(jp_lat_lon, df_pref_map, on="都道府県名")
df_pref_map["一人当たり賃金（相対値）"] = (
    df_pref_map["一人当たり賃金（万円）"]
    - (df_pref_map["一人当たり賃金（万円）"].min())
) / ((df_pref_map["一人当たり賃金（万円）"]).max() - (df_pref_map["一人当たり賃金（万円）"].min()))

view = pdk.ViewState(longitude=139.691648, latitude=35.689185, zoom=4, pitch=20)
